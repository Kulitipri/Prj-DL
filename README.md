...
1. **Input Layer**: Nhận ảnh kích thước `(256, 256, 3)`.
...
5. **Flatten Layer**: Duỗi dữ liệu 2D thành vector 1D.
6. **Dense Layer (Fully Connected)**: 128 nơ-ron, hàm kích hoạt `ReLU`.
7. **Dropout**: Tỷ lệ 0.5 để giảm thiểu Overfitting.
8. **Output Layer**: 25 nơ-ron (tương ứng 25 lớp), hàm kích hoạt `Softmax` để đưa ra xác suất phân loại.
...
1. **Accuracy**: Tính độ chính xác tổng thể (thường đạt >98%).
2. **Confusion Matrix**: Vẽ biểu đồ nhiệt (heatmap) để xem sự nhầm lẫn giữa các lớp.
3. **Classification Report**: Chi tiết Precision, Recall, F1-Score cho từng dòng mã độc.
4. **ROC Curves**: Vẽ đường cong ROC cho từng lớp để đánh giá khả năng phân loại.
5. **Misclassified Images**: Hiển thị trực quan các hình ảnh mà mô hình dự đoán sai để phân tích.
